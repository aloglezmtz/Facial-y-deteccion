...
Dibuja un gato CALICÓ (blanco + naranja + carey) en SVG puro y lo anima con CSS.
El pelaje calicó es fijo (no cambia con la emoción, igual que en un gato real);
lo que cambia según la emoción es la expresión (ojos/boca/extras) y la
intensidad de la animación, según la confianza de la fusión.
"""

from emociones_config import COLOR_CALICO
...
def generar_gatito_html(emocion: str, confianza: float = 0.5, tamano: int = 220) -> str:
    ...
    base = COLOR_CALICO["base"]
    naranja = COLOR_CALICO["mancha_naranja"]
    carey = COLOR_CALICO["mancha_carey"]
    ...
    <div class="gato-wrap">
        <svg width="{tamano}" height="{tamano}" viewBox="0 0 200 200">
            <defs>
                <clipPath id="clip-cuerpo">
                    <circle cx="100" cy="110" r="70"/>
                </clipPath>
            </defs>

            <path class="cola" d="M 165,170 Q 195,150 180,120" stroke="{carey}" stroke-width="14" fill="none" stroke-linecap="round"/>
            <path class="oreja-izq" d="M 50,70 L 35,25 L 80,55 Z" fill="{naranja}"/>
            <path class="oreja-der" d="M 150,70 L 165,25 L 120,55 Z" fill="{carey}"/>
            <path d="M 55,60 L 42,35 L 68,52 Z" fill="#FFE1D6"/>
            <path d="M 145,60 L 158,35 L 132,52 Z" fill="#FFE1D6"/>

            <!-- Cuerpo calicó: base crema + parches naranja/carey, recortados al círculo -->
            <circle cx="100" cy="110" r="70" fill="{base}"/>
            <g clip-path="url(#clip-cuerpo)">
                <ellipse cx="58" cy="72" rx="40" ry="32" fill="{naranja}" opacity="0.92"/>
                <ellipse cx="148" cy="118" rx="32" ry="46" fill="{carey}" opacity="0.88"/>
                <ellipse cx="95" cy="168" rx="36" ry="22" fill="{naranja}" opacity="0.75"/>
                <ellipse cx="45" cy="140" rx="20" ry="26" fill="{carey}" opacity="0.55"/>
            </g>
